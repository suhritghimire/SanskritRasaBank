...
"""
============================================================
NAVARASA CLASSIFICATION — MuRIL Training (High-Accuracy v2)
============================================================
Trains Google's MuRIL-large-cased model on Sanskrit Rasa
classification. Key improvements over baseline:

  1. Focal Loss       — focuses on hard/minority classes
  2. LLRD             — layer-wise learning rate decay
  3. Label Smoothing  — handles LLM annotation noise
  4. Live F1 Metrics  — tracks macro/weighted F1 per epoch
  5. Best on F1       — saves best model by weighted F1
  6. Higher Patience  — avoids premature early stopping
  7. Per-class Report — full confusion matrix at fold end
...
MAX_LENGTH = 256
FOCAL_GAMMA = 2.0      # Focal loss: focuses on hard examples
LABEL_SMOOTHING = 0.1  # Regularises against noisy LLM labels
LLRD_FACTOR = 0.95     # Layer-wise LR decay per BERT layer
...
MODELS_CONFIG = [
    {
        ...
        'learning_rate': 1e-5,              # Slower LR for large model
        'epochs': 12,
        'weight_decay': 0.01,
...
class FocalLoss(nn.Module):
    """
    Focal Loss: down-weights well-classified examples so the model 
    ...
    def forward(self, logits, labels):
        # Apply label smoothing via CrossEntropy first
...
def get_llrd_optimizer_params(model, base_lr, llrd_factor=0.95):
    """
    Returns optimizer parameter groups with decaying learning rates.
    Top BERT layers get base_lr, deeper layers get progressively lower LRs.
    """
    ...
    # BERT layers — 24 layers for muril-large
    num_layers = 24
...
class CustomTrainer(Trainer):
    """Trainer with Focal Loss + Label Smoothing + LLRD."""
    ...
    def create_optimizer(self):
        """Override to use LLRD optimizer groups."""
...
def make_compute_metrics(id2label):
    """Returns a closure for HuggingFace Trainer compute_metrics."""
...
def main():
    print("=" * 60)
    print("NAVARASA CLASSIFICATION — MuRIL Fine-tuning v2")
    print("  Focal Loss | LLRD | Label Smoothing | Live F1")
    print("=" * 60)
    ...
    if df['label'].isna().any():
        print("  Warning: Unmapped labels found:")
    ...
    while True:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
            ...
            training_args = TrainingArguments(
                ...
                metric_for_best_model="eval_weighted_f1",  # Optimise for F1
            ...
            trainer = CustomTrainer(
                ...
                callbacks=[EarlyStoppingCallback(early_stopping_patience=4)]  # More patience
            )

            # Resume from mid-epoch checkpoint if available
